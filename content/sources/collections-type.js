...
import { CONTENT_BASE, ARC_ACCESS_TOKEN,resizerKey } from 'fusion:environment'
...
const fetch = async (key = {}) => {
  ...
  const getStoriesWithFilter = async (from) => {
    try {
      const response = await axios.get(`${CONTENT_BASE}/content/v4/collections`, {
        params: {
          ...qs,
          from: from
        },
      ...
      const filteredResp = await filterByContentType(response.data);
      return filteredResp;
    } catch (error) {
